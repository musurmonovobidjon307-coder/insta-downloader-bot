...
async def download_handler(message: types.Message):
    ...
    try:
        ...
        builder = types.InlineKeyboardMarkup(inline_keyboard=[
            [types.InlineKeyboardButton(text="🎵 Qo'shig'ini topish", callback_data="audio_req")]
        ])
...
async def process_callback(callback: types.CallbackQuery):
    if callback.data == "audio_req":
        ...
        try:
            # "Link: " so'zidan keyingi qismni ajratamiz
            url = caption.split("Link: ")[1].strip()
            await callback.answer("Musiqa tayyorlanmoqda... 🎶")
            
            a_temp = f"temp_{callback.from_user.id}"
            audio_opts = {
                **YDL_OPTIONS,
            ...
            if os.path.exists(audio_file):
                await callback.message.answer_audio(types.FSInputFile(audio_file), caption="Marhamat! 🎵")
